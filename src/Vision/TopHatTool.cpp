...
TopHatTool::TopHatTool() {
    m_name = "顶帽变换";
}

bool TopHatTool::configure(const QJsonObject& params) {
...
bool TopHatTool::execute(const cv::Mat& input, ToolResult& result) {
    if (input.empty()) {
        result.ok = false;
        return false;
    }
    ...
    // 顶帽变换：原图减去开运算结果，提取亮的小区域
    cv::Mat dst;
    cv::morphologyEx(gray, dst, cv::MORPH_TOPHAT, element);
...
QJsonObject TopHatTool::serialize() const {
    QJsonObject obj;
    obj["id"] = m_id;
    obj["type"] = type();
    obj["name"] = m_name;
...
bool TopHatTool::deserialize(const QJsonObject& data) {
