...
bool OpeningTool::configure(const QJsonObject& params) {
...
bool OpeningTool::execute(const cv::Mat& input, ToolResult& result) {
    if (input.empty()) {
        result.ok = false;
        return false;
    }
...
QJsonObject OpeningTool::serialize() const {
    QJsonObject obj;
    obj["id"] = m_id;
    obj["type"] = type();
    obj["name"] = m_name;
...
bool OpeningTool::deserialize(const QJsonObject& data) {
