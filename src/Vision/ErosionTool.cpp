...
bool ErosionTool::configure(const QJsonObject& params) {
...
bool ErosionTool::execute(const cv::Mat& input, ToolResult& result) {
    if (input.empty()) {
        result.ok = false;
        return false;
    }
...
QJsonObject ErosionTool::serialize() const {
    QJsonObject obj;
    obj["id"] = m_id;
    obj["type"] = type();
    obj["name"] = m_name;
...
bool ErosionTool::deserialize(const QJsonObject& data) {
