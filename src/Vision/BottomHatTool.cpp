...
BottomHatTool::BottomHatTool() {
    m_name = "底帽变换";
}

bool BottomHatTool::configure(const QJsonObject& params) {
...
bool BottomHatTool::execute(const cv::Mat& input, ToolResult& result) {
    if (input.empty()) {
        result.ok = false;
        return false;
    }
    ...
    // 底帽变换：闭运算结果减去原图，提取暗的小区域
    cv::Mat dst;
    cv::morphologyEx(gray, dst, cv::MORPH_BLACKHAT, element);
...
QJsonObject BottomHatTool::serialize() const {
    QJsonObject obj;
    obj["id"] = m_id;
    obj["type"] = type();
    obj["name"] = m_name;
...
bool BottomHatTool::deserialize(const QJsonObject& data) {
