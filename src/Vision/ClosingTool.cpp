...
ClosingTool::ClosingTool() {
    m_name = "闭运算";
}

bool ClosingTool::configure(const QJsonObject& params) {
...
bool ClosingTool::execute(const cv::Mat& input, ToolResult& result) {
    if (input.empty()) {
        result.ok = false;
        return false;
    }
    ...
    // 闭运算：先膨胀后腐蚀
    cv::Mat dst;
    cv::morphologyEx(gray, dst, cv::MORPH_CLOSE, element);
...
QJsonObject ClosingTool::serialize() const {
    QJsonObject obj;
    obj["id"] = m_id;
    obj["type"] = type();
    obj["name"] = m_name;
...
bool ClosingTool::deserialize(const QJsonObject& data) {
